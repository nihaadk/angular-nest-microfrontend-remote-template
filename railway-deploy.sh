#!/usr/bin/env bash
#
# railway-deploy.sh — deploys this configured remote to Railway as its own,
# isolated project (two services: <name>-fe and <name>-be).
#
# Run this AFTER setup.sh has already configured fe/ and be/ - it reads the
# remote's name from fe/federation.config.mjs, so it can never disagree
# with what setup.sh already set.
#
...
NAME="$(sed -n "s/.*name: '\([^']*\)'.*/\1/p" "$FE_DIR/federation.config.mjs" | head -1)"
if [[ -z "$NAME" ]]; then
...
echo "→ Deploying '$NAME' to Railway ($FE_SERVICE + $BE_SERVICE)"

# --- Project + backend service ---------------------------------------------
# Whether be/ ITSELF (not this script's own directory - it may be linked to
# something unrelated, e.g. a shared project from prior manual work) is
# already linked is what makes re-running this script safe: linked -> reuse
# it, unlinked -> `--new` creates a fresh, isolated project.
if (cd "$BE_DIR" && railway status --json >/dev/null 2>&1); then
  echo "→ $BE_DIR is already linked to a Railway project, reusing it"
  echo "→ Deploying $BE_SERVICE"
  STATUS_JSON="$(cd "$BE_DIR" && railway status --json 2>/dev/null)"
  PROJECT_ID="$(echo "$STATUS_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
  ENVIRONMENT_ID="$(echo "$STATUS_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["environments"]["edges"][0]["node"]["id"])')"
  (cd "$BE_DIR" && railway up -y --service "$BE_SERVICE" --project "$PROJECT_ID" --environment "$ENVIRONMENT_ID" .)
else
  echo "→ Creating a new, isolated Railway project '$NAME' and deploying $BE_SERVICE"
  (cd "$BE_DIR" && railway up --new -y --name "$NAME" --service "$BE_SERVICE" .)
  STATUS_JSON="$(cd "$BE_DIR" && railway status --json 2>/dev/null)"
  PROJECT_ID="$(echo "$STATUS_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
  ENVIRONMENT_ID="$(echo "$STATUS_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["environments"]["edges"][0]["node"]["id"])')"
fi
...
# --- Frontend service --------------------------------------------------------
STATUS_JSON="$(railway status --json --project "$PROJECT_ID" --environment "$ENVIRONMENT_ID" 2>/dev/null || echo "$STATUS_JSON")"
HAS_FE="$(echo "$STATUS_JSON" | python3 -c "
import json, sys
d = json.load(sys.stdin)
names = [s['node']['serviceName'] for e in d['environments']['edges'] for s in e['node']['serviceInstances']['edges']]
print('$FE_SERVICE' in names)
")"

if [[ "$HAS_FE" != "True" ]]; then
  echo "→ Creating $FE_SERVICE"
  railway add --service "$FE_SERVICE" --project "$PROJECT_ID" --json >/dev/null
fi
echo "→ Deploying $FE_SERVICE"
(cd "$FE_DIR" && railway up -y --service "$FE_SERVICE" --project "$PROJECT_ID" --environment "$ENVIRONMENT_ID" .)
...
✅ '$NAME' deployed to Railway

  Frontend: https://$FE_DOMAIN
  Backend:  https://$BE_DOMAIN

Add this to the host shell's REMOTES_JSON variable:

  {"$NAME":"https://$FE_DOMAIN/remoteEntry.json"}
EOF
